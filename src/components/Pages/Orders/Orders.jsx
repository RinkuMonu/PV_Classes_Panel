...
export default function Orders() {
  ...
  const [ordersData, setOrdersData] = useState([]); // Initialize with static data
  ...
  const filteredOrders = ordersData.filter(order => {
    const matchesSearch = order.shipping_address.full_name.toLowerCase().includes(searchTerm.toLowerCase())
    const matchesStatus = !statusFilter || statusFilter === "all" ||
      order.status.toLowerCase() === statusFilter.toLowerCase()
    const matchesMethod = !methodFilter || methodFilter === "all" ||
      order.payment_method.toLowerCase() === methodFilter.toLowerCase()
  ...
  const changeStatus = async (order_id, status) => {
    try {
      const response = await axios.put(
        `${url}/api/orders/status/${order_id}`,
        {
          status: status,
        },
        {
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="mb-6">
            ...
            <div className="flex flex-wrap gap-4 mb-4 bg-white p-4 rounded">
              ...
              <button className="bg-emerald-600 hover:bg-emerald-700 text-white flex items-center px-4 py-2 ">
            ...
            <div className="flex flex-wrap gap-4 items-end bg-white p-4 rounded">
              ...
              <button 
                className="px-8 py-2  border border-gray-300 bg-transparent hover:bg-gray-50"
                onClick={() => {
                  setSearchTerm("")
                  setStatusFilter("")
                  setMethodFilter("")
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="text-xs font-semibold tracking-wide text-left text-gray-500 uppercase border-b border-gray-200 dark:border-gray-700 bg-gray-100 dark:text-gray-400 dark:bg-gray-800">
                  <tr>
                    <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">SR.NO</th>
                    ...
                    <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">INVOICE</th>
                  </tr>
                </thead>
                ...
                {currentOrders.length > 0 ? (
                  currentOrders.map((order, index) => (
                    <tr key={order._id} className="hover:bg-gray-50">
                      <td className="px-6 py-4 whitespace-nowrap font-medium text-sm text-blue-600">{index + 1}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <select onChange={(e)=> changeStatus(order._id,e.target.value)}
                          defaultValue={order.status}
                          className="w-24 h-8 text-sm rounded-md border border-gray-300 py-1 px-2 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500"
                        >
                          <option value="pending">Pending</option>
                          <option value="delivered">Delivered</option>
                          <option value="processing">Processing</option>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex gap-2">
                          <Tippy content="Print Reciept">
                            <button className="h-8 w-8 p-0 inline-flex items-center justify-center rounded-md text-gray-500 hover:bg-gray-100">
                              <Printer className="h-4 w-4" />
                            </button>
                          </Tippy>
                          <Tippy content="View Invoice">
                            <button onClick={() => handleViewInvoice(order._id)} className="h-8 w-8 p-0 inline-flex items-center justify-center rounded-md text-gray-500 hover:bg-gray-100">
